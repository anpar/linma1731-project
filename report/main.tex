...
linearization of the system. Our system can be represented as :
...
In order to linearize this system, the Jacobian of $F$ evaluated at the current
predicted state needs to be computed. For each time step $k$, this can be written as
\begin{equation*}
    F_k = 
    \begin{pmatrix}
    ...
    \end{pmatrix}
\end{equation*}
With $x_0 \sim\mathcal{N}(\vec{\mu}_0, \vec{P}_0 = \sigma^2_0\vec{I}_3)$, $u_k
\sim\mathcal{N}(\vec{0}, \vec{Q} = \vec{\Gamma}\sigma^2_u)$ and $w_k
\sim\mathcal{N}(0, R = \sigma^2_m)$, the linearized system is also Gaussian.
The Kalman Filter's equations can thus directly
be applied. The prediction step is written as follows:
...
Figure \ref{fig:ekf-results} shows two realizations of our Extended Kalman Filter with
$\sigma_u^2 = 0.01$, more precisely two trajectories of the $x$ coordinate and the
corresponding estimation errors (the same error metric has been used as for the particle
filtering).\\

Notice that one filtering succeeds quite well, whereas the second one fails for the time
steps in the range $[1300, 1500]$. This emphasizes that the algorithm doesn't always
converge as well as the particle filter. Besides, the mean errors, even for the best
realization, clearly indicate that the EKF yields less good results than particle
filtering (of about one order of magnitude). That being said, it is important to stress
that the EKF runs significantly faster than the particle filter.

\begin{figure}
    \centering
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{figures/ekf-good-x-trajectory}
        \caption{Trajectory of the $x$ coordinate - Good filtering} 
    \end{subfigure}%
    ~
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{figures/ekf-bad-x-trajectory}
        \caption{Trajectory of the $y$ coordinate - Bad filtering} 
    \end{subfigure}\\
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{figures/ekf-good-error}
        \caption{Error values - Good filtering} 
    \end{subfigure}
    ~
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{figures/ekf-bad-error}
        \caption{Error values - Bad filtering} 
    \end{subfigure}\\

    \caption{Trajectories and error values for two realizations of the EKF}
...
filtering. In both cases, cumulative distributive functions have the
...
for EKF by design of the Kalman filter.
...
\begin{figure}
    ...
    \caption{Space CDF distributions for CSMC and EKF filtering at $t = 5$s}
    \label{fig:space-cdf}
\end{figure}
