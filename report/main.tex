...
Figure \ref{fig:ekf-results} shows two realizations of our Extended Kalman Filter with $\sigma_u^2 = 0.01$, more precisely the trajectories of the $x$ coordinate and the error values (the same error metric has been used as for the particle filtering).\\

Notice that one filtering succeeds quite well, whereas the second one fails for the time steps in the range $[1300, 1500]$. This emphasizes that the algorithm doesn't always converge as well as the particle filter. Besides, the mean errors, even for the best realization, clearly indicate that the EKF yields less good results than particle filtering (of about one order of magnitude). This said, it is important to stress that the EKF runs significantly faster than the particle filter.
\begin{figure}
    \centering
    \begin{subfigure}{0.49\textwidth}
...
Figure \ref{fig:space-cdf} shows the space distributions for both CSMC and EKF filtering. TODO COMMENT.
